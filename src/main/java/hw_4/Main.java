...
//        }
        ...
        try (Statement st = connection.createStatement()){
            st.execute("""
                    CREATE TABLE IF NOT EXISTS `studentsGB`(
                    `id` bigint PRIMARY KEY,
        ...
        try (Statement st = connection.createStatement()){
            st.execute("""
                    INSERT INTO `studentsGB`(`id`,`first_name`, `second_name`,`group_id`) VALUES
                    (1, 'Ivan', 'Ivanov',1),
                    (2, 'Petr', 'Petrov',2),
                    (3, 'Fedor', 'Fedorov',3);
                    """);
    ...
    private static void run(Connection connection) throws SQLException {
        ...
        try (SessionFactory sessionFactory = configuration.buildSessionFactory()) {
            try(Session session = sessionFactory.openSession()){
                StudentsGB students = session.find(StudentsGB.class, 1L);
                System.out.println(students);
            }

            StudentsGB newStudent = new StudentsGB();
            newStudent.setId(5L);
            ...
            try(Session session = sessionFactory.openSession()){
                StudentsGB students = session.find(StudentsGB.class, 4L);
                System.out.println(students);
            }
        }
